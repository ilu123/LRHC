...
package com.dornbachs.zebra.views;
...
public class PaintView extends View {
        ...
        newState._paintMask = new byte[n];
        {
            ...
            for (int i2 = 0; i2 < PROGRESS_SCAN; i2++) {
                ...
                for (int i = iStart; i < iEnd; i++) {
                    ...
                    newState._paintMask[i] = (alpha < ALPHA_TRESHOLD ? (byte) 1 : (byte) 0);
                    pixels[i] = alpha << 24;
                }
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (_isPan) {
            LinearLayout.LayoutParams lp = (android.widget.LinearLayout.LayoutParams) this.getLayoutParams();
    ...
    private synchronized void paint(int x, int y) {
        int pos = y * _state._width + x;
        if (pos >= _state._pixels.length || pos < 0)
            return;
        int ispaint = (_state._pixels[pos]);
        //Log.d("pppp", "]]]]]] "+ispaint);
        if (ispaint != -1 && ispaint == _state._color)
            return;
        // Copy the original mask to the working mask because it will be
        // modified.
        System.arraycopy(_state._paintMask, 0, _state._workingMask, 0, _state._width * _state._height);

        // Do the nasty stuff.
        FloodFill.fillRaw(x, y, _state._width, _state._height, _state._workingMask, _state._pixels, _state._color);

        // And now copy all the pixels back.
        _state._paintedBitmap.setPixels(_state._pixels, 0, _state._width, 0, 0, _state._width, _state._height);
        invalidate();
    }
