...
public class DragSortListView extends ListView {
	...
	public DragSortListView(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			if (useDefault) {
				...
				DragSortController controller = new DragSortController(this, dragHandleId, dragInitMode, removeMode, clickRemoveId,
				        flingHandleId);
	...
	/**
	 * For each DragSortListView Listener interface implemented by <code>adapter</code>, this method
	 * calls the appropriate
	 * set*Listener method with <code>adapter</code> as the argument.
	 * 
	 * @param adapter
	 *        The ListAdapter providing data to back
	 *        DragSortListView.
	 * 
	 ...
	 * @param top
	 * @param height
	 *        Height of item at position. If -1, this function
	 ...
	 * 
	 * @param from
	 *        Position to move (NOTE: headers/footers ignored!
	 *        this is a position in your input ListAdapter).
	 * @param to
	 *        Target position (NOTE: headers/footers ignored!
	 *        this is a position in your input ListAdapter).
	 */
	...
	/**
	 * Cancel a drag. Calls {@link #stopDrag(boolean, boolean)} with <code>true</code> as the first
	 * argument.
	 */
	...
	private boolean mCancelForUserDo = false;
	protected void onClickRemoveButton(int which) {
		// end it
		if (mRemoveListener != null) {
			mCancelForUserDo = mRemoveListener.remove(which);
		}
		if (mCancelForUserDo) {
			return;
		}else{
	 ...
	 * 
	 * @param upperFrac
	 *        Fraction of ListView height for up-scroll bound.
	 *        Capped at 0.5f.
	 * @param lowerFrac
	 *        Fraction of ListView height for down-scroll bound.
	 *        Capped at 0.5f.
	 * 
	 */
	 ...
	 *        top).
	 * 
	 * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail
	...
	public boolean startDrag(int position, View floatView, int dragFlags, int deltaX, int deltaY) {
		if (mDragState != IDLE || !mInTouchEvent || mFloatView != null || floatView == null || !mDragEnabled) { return false; }
	 ...
	 * your own is not passed, the default {@link SimpleFloatViewManager} implementation is used.
	 */
	public interface FloatViewManager {
		 ...
		 * 
		 * @param position
		 *        Position of item to drag (NOTE: <code>position</code> excludes header Views; thus,
		 *        if you
		 ...
		 * of the float View can be altered by setting <code>location.x</code> and
	 ...
	 * 
	 * @param enabled
	 *        Set <code>true</code> to enable list
	 *        item dragging
	 */
	 ...
	 * for you; doesn't make sense to. Make sure {@link BaseAdapter#notifyDataSetChanged()} or
	 * something like it is called
	 * in your implementation. Furthermore, if you have a choiceMode other than
	 * none and the ListAdapter does not return true for {@link ListAdapter#hasStableIds()}, you
	 * will need to call {@link #moveCheckState(int, int)} to move the check boxes along with the
	 * list items.
	 * 
	 * @param l
	 */
	 ...
	 * Make sure to call {@link BaseAdapter#notifyDataSetChanged()} or something like it
	 ...
	 * adapter which does not have stable IDs (see {@link ListAdapter#hasStableIds()}). This is
	 * because without IDs, the
	 * ListView has no way of knowing which items have moved where, and cannot
	 * update the check state accordingly.
	 * <p>
	 * A word of warning about a "feature" in Android that you may run into when dealing with
	 * movable list items: for an adapter that <em>does</em> have stable IDs, ListView will attempt
	 * to locate each item based on its ID and move the check state from the item's old position to
	 * the new position — which is all fine and good (and removes the need for calling this
	 * function), except for the half-baked approach. Apparently to save time in the naive algorithm
	 * used, ListView will only search for an ID in the close neighborhood of the old position. If
	 * the user moves an item too far (specifically, more than 20 rows away), ListView will give up
	 * and just force the item to be unchecked. So if there is a reasonable chance that the user
	 * will move items more than 20 rows away from the original position, you may wish to use an
	 * adapter with unstable IDs and call this method manually instead.
	 * 
	 * @param from
	 * @param to
	 */
	 ...
	 * underlying adapter which does not have stable IDs (see {@link ListAdapter#hasStableIds()}).
