...
static cv::Mat img;  // image container
...
JNIEXPORT void JNICALL Java_com_lrkj_utils_LrSocketSurfaceView_getNaviFrame
    ...
    const char *mapFile = "/mnt/sdcard/com.lrkj.ctrl/navMap.pgm";
    ...
    while (!StopSocket) {
        ...
        if (sendSignal(sokt, 1008)) {
            ...
            if (size > 0) {
                ...
                for (unsigned int i = 0; i + 1 < vec.size(); i += 2) {
                    ...
                    cv::circle(markLayer, cv::Point(px, py), 1, cv::Scalar(255), -1, CV_AA);
        ...
        cv::Mat combine = img - markLayer;

        cv::resize(combine, ioImg, ResImgSiz);
